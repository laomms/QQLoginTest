	...
	public static bool DeFriendMsg(byte[] byteIn)
		{
			...
			using (MemoryStream ms = new MemoryStream(byteIn))
			{
				try
				{
					...
					try
                    {
						string msg = result.MessageInfo[0].QQInfo.MsgInfo.MsgTextInfo.MsgContent.Message_Content.content;
...
#region 发送好友消息
		public static bool SendFriendMsg(long QQFromId, string Message)
		{
...
[ProtoContract]
	...
	public class MsgTextInfos
	{
		...
		[ProtoMember(2)]
		public MessageStruct MsgContent;
	}
	[ProtoContract]
	...
	[ProtoContract]
	[ProtoInclude(21, typeof(MessageStruct))]
	public interface FriendInterface
	{
		MessageContent MsgContent {get; set;}
		MessageType MsgType {get; set;}
		MessageOther MsgOther {get; set;}
	}
	[ProtoContract]
	public class MessageStruct : FriendInterface
	{
		MessageContent FriendInterface.MsgContent
		{
			get
			{
		...
		MessageType FriendInterface.MsgType
		{
			get
			{
		...
		MessageOther FriendInterface.MsgOther
		{
			get
			{
