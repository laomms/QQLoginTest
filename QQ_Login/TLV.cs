...
#region tlv001
		...
		{
			var bytes = new byte[] {0, 1}.Concat(DataList.GetRandByteArray(4)).ToArray();
...
#region tlv008 手机系统语言中文804
		public static byte[] tlv008()
		{
			...
			//Debug.Print("tlv008:" + retByte.Length.ToString + vbNewLine + BitConverter.ToString(retByte).Replace("-", ""))
			return retByte;
		}
#endregion
#region tlv018
...
#region tlv104
		...
		{
			var bytes = VieryToken2;
			byte[] bytesLen = BitConverter.GetBytes(Convert.ToInt16(bytes.Length)).Reverse().ToArray();
			var retByte = new byte[] {1, 4}.Concat(bytesLen).Concat(bytes).ToArray();
			//Debug.Print("tlv104:" + retByte.Length.ToString + vbNewLine + BitConverter.ToString(retByte).Replace("-", ""))
			return retByte;
		}
#endregion
#region tlv106
		public static byte[] tlv106(byte[] user, byte[] md5pass, byte[] md5_2pass, byte[] _TGTKey, string guid, byte[] times, int appId, byte[] QQUTF8)
		{
			var bytes = new byte[] {0, 4}.Concat(DataList.GetRandByteArray(4)).ToArray();
			...
			bytes = bytes.Concat(DataList.HexStrToByteArray(guid)).ToArray();
			if (appId < 0)
			{
...
#region tlv109 androidId
		public static byte[] tlv109(string androidId)
		{
			var bytes = DataList.HexStrToByteArray(androidId);
			byte[] bytesLen = BitConverter.GetBytes(Convert.ToInt16(bytes.Length)).Reverse().ToArray();
...
#region tlv116 验证码共用
		...
		{
			var bytes = new byte[] {0, 0xA, 0xF7, 0xFF, 0x7C, 0x0, 0x1, 0x4, 0x0, 0x1, 0x5F, 0x5E, 0x10, 0xE2}.ToArray();
			byte[] bytesLen = BitConverter.GetBytes(Convert.ToInt16(bytes.Length)).Reverse().ToArray();
			var retByte = new byte[] {1, 0x16}.Concat(bytesLen).Concat(bytes).ToArray();
			//Debug.Print("tlv116:" + retByte.Length.ToString + vbNewLine + BitConverter.ToString(retByte).Replace("-", ""))
			return retByte;
		}
#endregion
#region tlv141 手机营业商
...
#region tlv128 手机硬件信息
		public static byte[] tlv128(string model, string guid, string brands)
		{
			...
			bytes = bytes.Concat(BitConverter.GetBytes(Convert.ToInt16(DataList.HexStrToByteArray(guid).Length)).Reverse().ToArray()).ToArray();
			bytes = bytes.Concat(DataList.HexStrToByteArray(guid)).ToArray();
...
#region tlv154
		...
		public static byte[] tlv145(string guid)
		{
			var bytes = DataList.HexStrToByteArray(guid);
			byte[] bytesLen = BitConverter.GetBytes(Convert.ToInt16(bytes.Length)).Reverse().ToArray();
...
#region tlv16b
		...
		{
			var bytes = new byte[] {0, 1, 0, 0xB, 0x67, 0x61, 0x6D, 0x65, 0x2E, 0x71, 0x71, 0x2E, 0x63, 0x6F, 0x6D};
			byte[] bytesLen = BitConverter.GetBytes(Convert.ToInt16(bytes.Length)).Reverse().ToArray();
...
#endregion
#region tlv188
		public static byte[] tlv188(string AndroidID)
		{
			var bytes = DataList.HexStrToByteArray(AndroidID);
			byte[] bytesLen = BitConverter.GetBytes(Convert.ToInt16(bytes.Length)).Reverse().ToArray();
...
#endregion
#region tlv202
		public static byte[] tlv202(string BSSID, string SSID)
		{
			var bytes = new byte[] {0, 0x10};
			bytes = bytes.Concat(DataList.HexStrToByteArray(BSSID)).ToArray();
...
#endregion
#region tlv511 长度加qq域名
		public static byte[] tlv511()
		{
			var bytes = new byte[] {0, 0xE, 1, 0, 0xA};
			bytes = bytes.Concat(Encoding.UTF8.GetBytes("tenpay.com")).ToArray();
			bytes = bytes.Concat(new byte[] {1, 0, 0x11}).ToArray();
			bytes = bytes.Concat(Encoding.UTF8.GetBytes("openmobile.qq.com")).ToArray();
			bytes = bytes.Concat(new byte[] {1, 0, 0xB}).ToArray();
			bytes = bytes.Concat(Encoding.UTF8.GetBytes("docs.qq.com")).ToArray();
			bytes = bytes.Concat(new byte[] {1, 0, 0xE}).ToArray();
			bytes = bytes.Concat(Encoding.UTF8.GetBytes("connect.qq.com")).ToArray();
			bytes = bytes.Concat(new byte[] {1, 0, 0xC}).ToArray();
			bytes = bytes.Concat(Encoding.UTF8.GetBytes("qzone.qq.com")).ToArray();
			bytes = bytes.Concat(new byte[] {1, 0, 0xA}).ToArray();
			bytes = bytes.Concat(Encoding.UTF8.GetBytes("vip.qq.com")).ToArray();
			bytes = bytes.Concat(new byte[] {1, 0, 0x11}).ToArray();
			bytes = bytes.Concat(Encoding.UTF8.GetBytes("gamecenter.qq.com")).ToArray();
			bytes = bytes.Concat(new byte[] {1, 0, 0xA}).ToArray();
			bytes = bytes.Concat(Encoding.UTF8.GetBytes("qun.qq.com")).ToArray();
			bytes = bytes.Concat(new byte[] {1, 0, 0xB}).ToArray();
			bytes = bytes.Concat(Encoding.UTF8.GetBytes("game.qq.com")).ToArray();
			bytes = bytes.Concat(new byte[] {1, 0, 0xC}).ToArray();
			bytes = bytes.Concat(Encoding.UTF8.GetBytes("qqweb.qq.com")).ToArray();
			bytes = bytes.Concat(new byte[] {1, 0, 0x9}).ToArray();
			bytes = bytes.Concat(Encoding.UTF8.GetBytes("ti.qq.com")).ToArray();
			bytes = bytes.Concat(new byte[] {1, 0, 0xD}).ToArray();
			bytes = bytes.Concat(Encoding.UTF8.GetBytes("office.qq.com")).ToArray();
			bytes = bytes.Concat(new byte[] {1, 0, 0xB}).ToArray();
			bytes = bytes.Concat(Encoding.UTF8.GetBytes("mail.qq.com")).ToArray();
			bytes = bytes.Concat(new byte[] {1, 0, 0xA}).ToArray();
			bytes = bytes.Concat(Encoding.UTF8.GetBytes("mma.qq.com")).ToArray();
			byte[] bytesLen = BitConverter.GetBytes(Convert.ToInt16(bytes.Length)).Reverse().ToArray();
			var retByte = new byte[] {5, 0x11}.Concat(bytesLen).Concat(bytes).ToArray();
...
#region tlv525
		public static byte[] tlv525(int flag, byte[] time, int Appid) //1=普通登录 2=假锁登录
		{
			...
			else
			{
				...
				bytes = bytes.Concat(BitConverter.GetBytes(Appid).Reverse().ToArray()).ToArray();
				bytes = bytes.Concat(new byte[] {0, 0, 0, 0}).ToArray();
				bytes = bytes.Concat(DataList.GetRandByteArray(4)).ToArray();
				bytes = bytes.Concat(new byte[] {4}).ToArray();
				bytes = bytes.Concat(new byte[] {0x65, 0x56, 0xAB, 0xB}).ToArray(); // 参数还未知,等等以后解
				bytes = bytes.Concat(time).ToArray();
				bytes = bytes.Concat(BitConverter.GetBytes(Appid).Reverse().ToArray()).ToArray();
				...
				bytes = bytes.Concat(DataList.GetRandByteArray(3)).ToArray();
				bytes = bytes.Concat(new byte[] {4}).ToArray();
...
#endregion
#region tlv544
		public static byte[] tlv544() //1=普通登录 2=假锁登录
		{
			byte[] bytes = new byte[] {0, 0, 7, 0xD9, 0, 0, 0, 0, 0, 0x2E, 0, 0x20};
			bytes = bytes.Concat(DataList.GetRandByteArray(32)).ToArray();
			...
			bytes = bytes.Concat(DataList.GetRandByteArray(15)).ToArray();
